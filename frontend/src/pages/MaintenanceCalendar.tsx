...
export function MaintenanceCalendar() {
  ...
  return <Shell title="维修日历" subtitle="车辆维保计划、历史费用和逾期风险集中排查。">
    ...
    <div className="panel" style={{ marginTop: 16 }}>{records.map((r) => {
      const overdue = isOverdue(r.nextDate);
      return <div className="row" key={r.id} style={overdue ? { backgroundColor: '#fef2f2' } : {}}>
        <span>
          {getVehiclePlate(r.vehicleId)} · {r.vendor} · {r.items.join('、')}
          {overdue && <span className="danger" style={{ marginLeft: 8 }}>（已逾期，下次应于 {r.nextDate}）</span>}
          {!overdue && <span className="muted" style={{ marginLeft: 8 }}>（下次：{r.nextDate}）</span>}
        </span>
